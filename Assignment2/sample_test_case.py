...
def test_case():
    ...
    #except:
        #print("SAMPLE TEST CASE 1 FOR THE DFS_TRAVERSAL FAILED")
    #try:
    if (tri_traversal(cost,heuristic, 4, [6, 7, 10]))[0] == [4,1,2,6]:
        print("SAMPLE TEST CASE 4 FOR THE  DFS_TRAVERSAL PASSED")
    else:
        print("SAMPLE TEST CASE 4 FOR THE  DFS_TRAVERSAL FAILED")
    ...
    if (tri_traversal(cost,heuristic, 1, [6, 7, 10]))[2] == [1, 5, 4, 7]:
        ...
        #print(tri_traversal(cost,heuristic, 1, [6, 7, 10])[2])
    '''    
    try:
        if (tri_traversal(cost,heuristic, 1, [6, 7, 10]))[2] == [1, 5, 4, 7]:
            print("SAMPLE TEST CASE 3 FOR THE  A_star_TRAVERSAL PASSED")
